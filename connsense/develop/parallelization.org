...
def reindex(tap, inputs, variables):
    ...
    return(
        ...
        .apply(DataCall))
#+end_src
#+name: develop-parallelization-inputs-reindex-datasets-0
#+begin_src python
...
def generate_inputs_0(of_computation, in_config):
    ...
    def tap_datasets(for_inputs, and_additionally=None):
        ...
        if and_additionally:
            ...
            references.update({
                key: {"dataset": ref} for key, ref in and_additionally.items()} or {})
    ...
    try:
        ...
        LOG.warning(
            "It seems no controls have been configured for the input of %s", of_computation)
        result = original
    else:
        ...
        if controls:
            ...
            controlled = pd.concat(
                [tap_datasets(for_input, and_additionally=to_tap).apply(datacall(shuffle))
            ...
            result = result.reorder_levels(
                [l for l in result.index.names if l != "control"] + ["control"])
        else:
            result = original
...
def symlink_pipeline_base(configs, at_dirpath):
    ...
    return {
        "pipeline": {
            fmt: symlink_to(config_at_path=p)
            for fmt, p in configs["pipeline"].items() if p},
        "runtime": {
            fmt: symlink_to(config_at_path=p)
            for fmt, p in configs["runtime"].items() if p}}
#+end_src
...
#+RESULTS: develop-parallelization-single-node-multiprocess-circuit
: None
...
def run_multinode(process_of, computation, in_config, using_runtime):
    ...
    if process_of == setup_compute_node:
        ...
        compute_nodes = {
            c: setup_compute_node(c, inputs, (computation_type, of_quantity, to_stage), using_configs)
            for c, inputs in batched.groupby("compute_node")}
        return {"configs": using_configs,
                "number_compute_nodes": n_compute_nodes, "number_total_jobs": n_parallel_jobs,
                "setup": write_multinode_setup(compute_nodes, inputs,  at_dirpath=to_stage)}
...
def write_pipeline_control(algorithm, at_dirpath): #pylint: disable=unused-argument
    ...
    if not control_config.exits():
        raise RuntimeError(
            f"InvalicComputeNode: {at_dirpath}."
            " The directory's parent is missing a control config.")
...
def input_circuit_args(computation, in_config, load_circuit=True, load_connectome=False):
    ...
    input_connectomes = computation_inputs.get("connectome", None)
    if input_connectomes:
        assert len(input_connectomes) == 1,\
            f"NotImplemented processing more than one connectome"
        x = input_connectomes[0]
    else:
        x = None
...
#+RESULTS: parallelize-single-nodel-circuit
: None

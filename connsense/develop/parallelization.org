...
A unit of computation will be that of a single subtarget, and batches will be run on separate compute nodes. Each parallel compute node will produce results on it's path that must be collected into the ~connsense-TAP~ store. So we have to implement three separate steps of a parallel processes: 1. distribute, 2. run, and 3. collect

* Describe the computation

Computations are entered as sections in the config. We will reference a computation by it's ~computation-type~ and the ~quantity~ it computes. We will enter computations of the same ~computation-type~ together in the config, enterting the computation parameters for each individual quantity.  Normally a process of ~(computation-type, quantity)~ should produce a frame or series that will be saved as a HDF dataset. In some cases there may be multiple datasets, each saved under their own dataset under the group ~<computation-type>/<quantity>~.  The names of these datasets will be read from the config.

Multiple groups for the same ~<computation-type>/<quantity>~ will apply to computations of ~extract-node-types~, which configures ~modeltypes~ to extract. Each ~modeltype~ will have components configured as dict mapping the component-name to a dict of applicable parameters, and a reference to the method to use for extraction.
...
A string representation of a computation will need a method to read it,
...
Once we have described a computation, we can parameterize it using the config. A ~quantity~ can be configured as a singleton, or with as a ~multicomp-quantity~ with components. Each component of a ~multicomp-quantity~ is represented as ~<quantity>/<component>~.
...
To run a computation of multiple inputs in parallel on multiple ~compute-nodes~, we will need to assign to each input the ~compute-node~ it will run on, and the parallel ~batch~ it will be queued in,

#+name: develop-parallelization-batched-inputs
#+begin_src python
def batch_multinode(computation, of_inputs, in_config, at_path, using_parallelization, single_submission=500):
    ...
    LOG.info("Group %s compute node into launchscript submissions", compute_nodes.compute_node.nunique())
    submissions = group_launchscript(assignment=compute_nodes, max_entries=single_submission)
...
#+RESULTS: develop-parallelization-estimate-batched-inputs-load
: None
...
def distribute_compute_nodes(parallel_batches, upto_number):
    ...
    assignment = pd.Series(compute_nodes[parallel_batches], name=dataset, index=parallel_batches.index)
    return assignment


def read_compute_nodes(at_dirpath):
    """..."""
    assignment_h5, dataset = COMPUTE_NODE_ASSIGNMENT
...
** 3. group compute nodes into submissions --- this becomes useful when only a fixed number of computations can be submitted to a cluster's queue at a time.


#+name: develop-parallelization-batched-inputs-group-launchscripts
#+begin_src python
def group_launchscripts(assignment, max_entries):
    """..."""
    if assignment.compute_node.nunique() <= max_entries:
        return assignment
...
In ~connsense-TAP~ we develop the concept of a ~unit-computation~. A single ~unit-computation~ will run as a single process from ~connsense-TAP~'s viewpoint. The compuation's method might have it's own mind, and run muliti-process. This can be setup using ~number_tasks_per_compute_node=1~ in the ~runtime-config~. This allows for external parallelization that ~connsense-TAP~'s parallelization model will not allow. To implment such a feature, we will have to manipulate the ~connsense-TAP-index~. Let us first define a ~unit-computation~.

For a given ~computation~, it's ~unit-computation~ is configured as the ~index~ entry. If an ~index~ is not configured, the configured ~input~ is used,
...
def read_index(of_computation, in_config):
    ...
    return parameters.get("index", paramters["input"])


def index_inputs(computation, to_tap):
    """..."""
    index_vars = read_index(computation, to_tap._config)
    if len(index_vars) > 1:
...
def slice_units(computation, to_tap):
    """..."""
    unit_computations = input_units(computation, to_tap)
    return [unit_computations[s:s+1] for s in range(0, len(unit_computations))]

#+end_src

If all input ~unit-computations~ could be determined from the configured ~index~, we would not have to open up the data of inputs just to batch them to setup parallel runs. This is possible if each computation's input data is available in the HDFstore. jHowever, certain computations' input data may be large a transform of a pervious ~connsense-TAP~ step. There is no reason to save these transformations in H5, as we can just transform the input before sending it to the computation.
...
So instead of returning a dataset with all it's members loaded, we will just return methods ready to do so when needed.

#+name: develop-parallelization-inputs-load-datasets
#+begin_src python
...
#+begin_src python
def index_units(computation, to_tap):
    """The index specified in the computaiton config.
    """
...
<<develop-parallelization-batched-inputs-load>>
...
<<develop-parallelization-inputs-load-datasets>>

...
def collect_node_population(setup, from_dirpath, in_connsense_store):
    ...
    def describe_output(of_compute_node):
        ...
        try:
            ...
            raise RuntimeError(f"No output configured for compute node {pformat(of_compute_node)}") from ferr
        return output
...
Depending on the scientific and computational complexity of an analysis, we may have to run a pipeline computation iteratively. After an initial run, and collect, we do not want to collect the results that are already in the main ~connsense-TAP-HDFStore~ after each /re-run/ of the computation. Either we ~clean-up~, or ~update~.
#+name: develop-parallelization-multinode-update-analyze-step
#+begin_src python
def update_analyze_step(setup, from_dirpath, in_connsense_store, slicing):
    """..."""
    try:
        with open(from_dirpath/"description.json", 'r') as f:
            analysis = json.load(f)
    ...
    master_store = in_store(connsense_h5, hdf_group)

    def find_update(compute_node, output):
        """..."""
        cn_connsense_h5, g = output
        cn_store = in_store(cn_connsense_h5, hdf_group=g)
        return cn_store.loc[cn_store.index.difference(master_store.index)]

    return master_store.collect({c: find_})
    return {c: move(compute_node=c, output=o) for c, o in outputs.items() if o}

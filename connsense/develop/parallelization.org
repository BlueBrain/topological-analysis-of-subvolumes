...
#+RESULTS: develop-parallelization-inputs
: None
...
def load_connsense_input(computation, in_config, with_name):
    ...
    if computation_type == "define-subtargets":
        ...
        return read((store, group), for_step=computation_type)

    key = f"{group}/{of_quantity}"

    if computation_type in ("extract-node-types", "extract-node-populations"):
        return read((store, key), for_step=computation_type)

    if computation_type == "extract-edge_populations":

        if dataset.endswith("/adj"):
            return read_toc_plus_payload((store, key)).rename(with_name)

        if dataset.endswith("/props"):
            return (matrices.get_store(store, key, for_matrix_type="pandas.DataFrame", in_mode='r').toc
                    .rename(with_name))
    ...
    if computation_type.startswith("analyze-"):
        return (matrices.get_store(store, key, for_matrix_type=parameters["output"], in_mode='r').toc.
                rename(with_name))
...
#+RESULTS: develop-parallelization-load-inputs
: None
...
#+begin_src python
def collect_batched_edge_populations(of_quantity, results, on_compute_node, hdf_group):
    """..."""
    ...
    def collect_batch(b, output):
        ...
        adj = write_toc_plus_payload(adj, (in_connsense_h5, f"{hdf_group}/adj"))

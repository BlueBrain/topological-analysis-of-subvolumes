#+PROPERTY: header-args:jupyter :session  ~/Library/Jupyter/runtime/active-ressh.json :pandoc t
#+PROPERTY: header-args:jupyter-python :session ~/Library/Jupyter/runtime/active-ressh.json :pandoc t
#+STARTUP: overview
...
def pour(tap, variables):
    """..."""
    LOG.info("Pour tap \n%s\n to get values for variables:]n%s", tap._root, pformat(variables))
    datasets = {var: load_dataset(tap, var, values) for var, values in variables.items()}
...
def setup_multinode(process, of_computation, in_config, using_runtime):
    ...
    computation_type, of_quantity = describe(of_comptuation)
    ...
    batched = batch_multinode(process, of_computation, inputs, in_config,
    ...
    if process == setup_compute_node:
        ...
        compute_nodes = {c: setup_compute_node(c, inputs, of_computation, using_configs)
...
def get_workspace(for_computation, in_config, in_mode=None):
    ...
    for_computation_type, of_quantity = describe(for_computation)
    rundir = workspace.get_rundir(in_config, step=for_computation, substep=of_quantity, in_mode=m)

#+end_src

To setup a multinode computation, we will write configs in the computation's workspace, and read them while collecting,
